...
class AgentSkill(BaseModel):
    ...
    id: str = Field(..., description="A unique identifier for the skill within the agent")
    name: str = Field(..., description="A human-readable name for the skill")
    description: str = Field(..., description="A detailed description of what the skill does")
    input_modes: List[str] = Field(default_factory=list, description="List of supported input content types")
    output_modes: List[str] = Field(default_factory=list, description="List of supported output content types")
    examples: Optional[List[Dict[str, Any]]] = Field(None, description="Example inputs and outputs for the skill")


class AuthenticationScheme(BaseModel):
    """Describes an authentication scheme required by the agent."""
    scheme: str = Field(..., description="The authentication scheme (e.g., 'Bearer', 'Basic', 'OAuth2')")
    description: Optional[str] = Field(None, description="Human-readable description of the authentication requirements")
...
class AgentCard(BaseModel):
    ...
    description: str = Field(..., description="A description of the agent's purpose and capabilities")
    url: str = Field(..., description="The base URL where the A2A server can be reached")
    provider: AgentProvider = Field(..., description="Information about the organization providing this agent")
...
class Task(BaseModel):
    ...
    title: Optional[str] = Field(None, description="A human-readable title for the task")
    description: Optional[str] = Field(None, description="A description of what the task is doing")
...
class Part(BaseModel):
    ...
    type: str = Field(..., description="The type of content (e.g., 'text', 'file', 'data')")
    content: Any = Field(..., description="The actual content of the part")
    metadata: Optional[Dict[str, Any]] = Field(None, description="Additional metadata for the part")
...
class JSONRPCRequest(BaseModel):
    ...
    id: Optional[Union[str, int]] = Field(None, description="Unique identifier for the request")


class JSONRPCResponse(BaseModel):
    """JSON-RPC 2.0 response structure."""
    jsonrpc: Literal["2.0"] = Field("2.0", description="JSON-RPC version")
    id: Optional[Union[str, int]] = Field(None, description="Identifier matching the request")
    result: Optional[Any] = Field(None, description="The result of the method call")
...
class JSONRPCError(BaseModel):
    ...
    data: Optional[Any] = Field(None, description="Additional information about the error")
...
class SendMessageRequest(BaseModel):
    ...
    task_id: Optional[str] = Field(None, description="Optional task ID to associate with the message")
    skill_id: Optional[str] = Field(None, description="Optional skill ID to use for processing")


class SendMessageResponse(BaseModel):
    """Response from sending a message."""
    task: Task = Field(..., description="The task created or updated by the message")
    message: Optional[Message] = Field(None, description="Optional response message")
...
class TaskStatusUpdateEvent(BaseModel):
    ...
    message: Optional[Message] = Field(None, description="Optional message associated with the update")
    final: bool = Field(False, description="Whether this is the final update for the task")
